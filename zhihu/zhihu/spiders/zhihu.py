...
class zihuSpider(scrapy.Spider):
    ...
    def start_requests(self):
        yield scrapy.Request(self.followees_url.format(user=self.start_user),callback=self.parse_urltoken)
        yield scrapy.Request(self.follower_url.format(user=self.start_user),callback=self.parse_urltoken)
    ...
    def parse_urltoken(self, response):
        ...
        if not pagi['is_end']:
                # print('next_page: %s'%pagi['next'])
                yield scrapy.Request(pagi['next'],callback=self.parse_urltoken)
